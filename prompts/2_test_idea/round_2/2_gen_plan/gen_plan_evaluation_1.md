...
### [1] SYSTEM-USER prompt · 2026-06-22 12:06:32 UTC
...
type: evaluation
objective: >-
  Assess robustness of the asymmetry finding to methodological choices, compute power analysis to determine what sample size
  would be needed for a genuine cross-linguistic test, and diagnose why iteration 1's 14-language extension failed (to inform
  readers on the state of verified spoken UD data).
approach: >-
  (1) ROBUSTNESS: Re-fit the per-language asymmetry analysis using alternative methodological choices: (a) no length normalization
  (raw MDD residuals); (b) length as a predictor rather than residualized; (c) Huber robust regression instead of OLS; (d)
  outlier sensitivity (remove top/bottom 1% of MDD values). Report whether the per-language asymmetry direction and sign remain
  stable across these variants. (2) POWER ANALYSIS: Given the observed effect size (Δ_asymmetry per language) and within-language
  variance, compute the sample size (number of languages) needed to achieve statistical significance (α=0.05, β=0.20) under
  the alternative hypothesis (true asymmetry ≠ 0). Report the result as: 'To achieve 80% power to detect an asymmetry of magnitude
  X, a sample of ~N languages would be required.' (3) CROSS-LANGUAGE AUDIT: Document which language families in commul/universal_dependencies
  have verified spoken-written UD treebank pairs. Report the final census: e.g., '2 Slavic (Slovenian), 1 Romance (French),
  0 Germanic native-spoken (en_gum conversational only), 0 others'. Explain why the 14-language extension failed (most were
  written genres). (4) HONEST SCOPE STATEMENT: Synthesize into a clear statement of what the paper demonstrates (argument-adjunct
  asymmetry in 2–3 verified languages, exploratory finding) vs. what remains open (cross-linguistic generalization on a larger
  verified sample). Output eval_out.json with {robustness_variants, power_analysis, language_family_census, scope_statement}
  and a figure showing the universe of published UD treebanks color-coded by whether they have verified spoken vs. written
  pairs.
depends_on:
- id: art_Gq_zeOShbi_1
...
### [2] HUMAN-USER prompt · 2026-06-22 12:06:32 UTC
...
### [3] SKILL-INPUT — aii-web-tools · 2026-06-22 12:07:24 UTC
