...
type: dataset
objective: >-
  Audit all 'spoken' treebanks in commul/universal_dependencies against official UD documentation, identify which are truly
  transcribed natural speech, and produce a verified list of spoken-written language pairs with standardized dependency metrics
  and corrected case-richness computation.
approach: >-
  For each language in commul/universal_dependencies marked as 'spoken' in the metadata: (1) Load the treebank and its metadata
  from HuggingFace; (2) Consult the official UD repository and accompanying papers/documentation (e.g., UD GitHub readmes,
  published treebank papers) to verify whether the corpus is transcribed speech or written text; (3) For genuinely spoken
  corpora, pair with a written counterpart; (4) For misidentified corpora (e.g., de_hdt = newspaper, ru_syntagrus = journalism),
  flag as 'not genuinely spoken' and exclude from analysis. (5) For English, attempt to load en_gum conversational/interview/vlog/speech
  genres (native spoken) and pair with a distinct written treebank (e.g., en_lines or en_partut, not en_ewt to avoid data
  overlap). If en_gum spoken is too sparse, acknowledge that native English spoken is unavailable. (6) Extract dependency
  arcs from verified pairs only, classify into ARGUMENT (nsubj, obj, iobj, ccomp, xcomp), ADJUNCT (advcl, acl, acl:relcl),
  MODIFIER (nmod, amod, advmod). (7) Recompute case richness using a single, explicitly stated procedure: proportion of NOUN
  + PRON tokens (UD UPOS, excluding PROPN) bearing a non-empty Case feature in the combined spoken+written corpus per language.
  Report token counts used for transparency. (8) Verify case-richness values against published typological resources (e.g.,
  WALS) for sanity check. Output data_out.json with only verified language pairs, and output a methodological report (audit_report.txt)
  documenting which treebanks were included/excluded and why.
depends_on: []
</artifact_direction>
...
### [3] SKILL-INPUT — aii-web-tools · 2026-06-22 12:06:56 UTC
