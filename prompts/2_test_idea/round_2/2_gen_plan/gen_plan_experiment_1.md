...
type: experiment
objective: >-
  Re-test the argument-adjunct asymmetry hypothesis using sentence-level aggregates (not arc-level) as the unit of analysis,
  with bootstrap confidence intervals that respect the clustered structure of the data, and per-language honest sample-size
  reporting.
approach: >-
  Load the verified spoken-written dataset from the DATASET artifact. (1) For each language pair, (2) for each language-modality-relation_category
  stratum, compute sentence-level MDD: aggregate per sentence by averaging dependency distances of arcs in that category.
  This produces one data point per sentence per category. (3) Apply sentence-length normalization at the sentence level: for
  each sentence, compute residuals from regressing sentence-level log(MDD) on log(sentence_length) within each language-modality
  stratum. (4) Compute bootstrap confidence intervals at the SENTENCE level: for each language, resample 1000 times with replacement
  at the sentence level (preserving the clustering structure), compute the mean Δ_MDD_residual (spoken minus written) per
  category, and extract 95% CI from the bootstrap distribution. Report per-language estimates with CIs. (5) Fit a per-language
  asymmetry index: Δ_adjunct_residual minus Δ_argument_residual for each language, with bootstrap CI. (6) Compute effect sizes
  (Cohen's d) at the sentence level. (7) Conduct a one-sample t-test on the per-language asymmetry indices (df = n_languages
  - 1) to test whether the asymmetry is significantly different from zero across the verified language set. Explicitly report
  n_languages (likely 2–3) and acknowledge that this is underpowered for cross-linguistic inference; frame as exploratory
  evidence. (8) Output method_out.json with per-language asymmetry estimates, 95% bootstrap CIs, effect sizes, t-test results,
  diagnostic plots (asymmetry index by language, bootstrap distributions per language), and explicit discussion of statistical
  power.
depends_on:
- id: art_F2XD0ACeTqeG
...
### [3] SKILL-INPUT — aii-web-tools · 2026-06-22 12:07:20 UTC
